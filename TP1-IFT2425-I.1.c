...
//--------------------------------------------------------------------------------
 ...
 for(int i=0;i<length;i++) for(int j=0;j<width;j++) Graph2D[i][j]=j/2.0;
  ...
  for (int k = 0; k < 512; k++) {
    // c = xk + j*yl --> k,l e [0, 511]
    xk = 2.0*(k - 512/1.35) / (511);
    for (int l = 0; l < 512; l++) {
      yl = 2.0*(l - 512/2.0) / (511);

      int iter = 0;
      double xn = 0.0;
      double yn = 0.0;
      while (iter++ < iterMax) {
        // xn1 et xn2 des variables temporaires
        double xn1 = pow(xn,2)-pow(yn,2)+xk;
        double yn1 = 2.0*xn*yn+yl;
        xn = xn1;
        yn = yn1;
        z = sqrt(pow(xn,2) + pow(yn,2)); //module de z (|z|)
        if (z > 2) break; // si non convergence, on arrête
      }
      //si non convergence (z > 2) : colorier en blanc (255)
      if (z > 2) {
        Graph2D[k][l] = 255;
      } else { //si convergence (z < 2) : colorier en noir (0)
        Graph2D[k][l] = 0;
      }
    }  
  }
